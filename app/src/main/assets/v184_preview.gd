...
func _process(delta: float) -> void:
    ...
    if not _checks_done and _preview_frames >= 10:
        ...
        if _v184_left_edge == null or _v184_right_edge == null or _v184_gate == null or _v184_window_label == null:
            ...
            get_tree().quit(15)
            return
        var easy := _v184_tolerance_cm(2.0, 0.2, 0.1)
        var hard := _v184_tolerance_cm(7.5, 2.2, 1.4)
        if easy <= hard or hard < 1.99 or easy > 8.01:
            push_error("Make-window difficulty regression")
            get_tree().quit(15)
            return
        _v165_recommended_offset = 0.42
        _v183_update({"distanceToCup": 5.4, "sideSlope": 1.65, "longSlope": -0.72, "running": false}, true)
        if not _v183_panel.visible or not _v184_left_edge.visible or _v184_left_edge.points.size() != _v183_path_line.points.size():
            push_error("Make-window live binding regression")
            get_tree().quit(15)
            return
        if _v184_gate.points.size() != 2 or _v184_window_label.text.find("MAKE WINDOW") < 0:
            push_error("Make-window gate regression")
            get_tree().quit(15)
            return
        var left_mid := _v184_left_edge.points[_v184_left_edge.points.size() / 2]
        var right_mid := _v184_right_edge.points[_v184_right_edge.points.size() / 2]
        if left_mid.distance_to(right_mid) < 10.0:
            push_error("Make-window corridor collapsed")
            get_tree().quit(15)
            return
