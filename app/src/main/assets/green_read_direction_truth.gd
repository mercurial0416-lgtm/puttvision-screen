...
func _overview_aim_is_off_map(offset_m: float) -> bool:
    return _overview_aim_is_valid(offset_m) and absf(offset_m) > OVERVIEW_AIM_VISUAL_SPAN_M
...
func _build_hud() -> void:
    ...
    for child in _v183_panel.get_children():
        if child is Label:
            ...
            if label.text == "gold = recommended read  •  teal = fall line":
                ...
                break
    _overview_aim_label = _v174_text(_v183_panel, Vector2(20, 222), Vector2(OVERVIEW_AIM_NORMAL_WIDTH, 18), "AIM CENTER", 10, Color("#f4dda0"))
    ...
    _overview_aim_marker.points = PackedVector2Array([Vector2(-5.5, 5.0), Vector2(0.0, 0.0), Vector2(5.5, 5.0)])
