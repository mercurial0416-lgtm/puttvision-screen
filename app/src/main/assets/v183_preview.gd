...
func _process(delta: float) -> void:
    ...
    if not _checks_done and _preview_frames >= 10:
        ...
        if curve.size() != 25 or curve[0].distance_to(_v183_ball.position) > 0.001 or curve[curve.size() - 1].distance_to(_v183_cup.position) > 0.001:
            push_error("Green overview path endpoint regression")
            get_tree().quit(14)
            return
        var mid := curve[curve.size() / 2]
        var center_x := V183_MAP_ORIGIN.x + V183_MAP_SIZE.x * 0.5
        if mid.x <= center_x or not _v183_panel.visible or _v183_distance_label.text != "5.4 m":
            push_error("Green overview live binding regression")
            get_tree().quit(14)
            return
        if _v183_break_label.text.find("L") < 0 or _v183_grade_label.text.find("UP") < 0:
            push_error("Green overview slope direction regression")
            get_tree().quit(14)
            return
