...
func _live_pair_is_finite(s: Dictionary, x_key: String, y_key: String) -> bool:
    return s.has(x_key) and s.has(y_key) \
        and is_finite(float(s.get(x_key, 0.0))) \
        and is_finite(float(s.get(y_key, 0.0)))
...
func _apply_snapshot(s: Dictionary, immediate: bool, delta: float) -> void:
    ...
    # When startX/startY are missing or non-finite, the first real measured ball position is the
    # only truthful presentation origin available. Patch only the Godot copy and update the inherited
    # HUD origin before the base live-break code sees the snapshot. If the first frame has no valid
    # ball sample either, keep TRACKING and resolve on the first finite sample instead of falling back
    # to (0, 0). This never feeds Android physics, aiming, scoring, or read advice.
