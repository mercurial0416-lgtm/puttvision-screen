...
func _process(delta: float) -> void:
    ...
    if probe._focus_replay_stage(0.719) != "TRAIL CAM":
        ...
        get_tree().quit(29)
        return
    if probe._focus_replay_stage(0.720) != "CUP CAM":
        push_error("Replay camera chapter handoff regression")
        probe.free()
        get_tree().quit(29)
        return
...
func _preview_add_replay_timeline(progress: float, alpha: float, bar_height: float, side_inset: float, label_width: float, stage_width: float, track_height: float, chapter_start: float, stage_text: String) -> void:
    var label := Label.new()
    ...
    marker.position = Vector2(track_left + track_width * clampf(chapter_start, 0.0, 1.0) - 1.0, 1080.0 - bar_height + 18.0)
