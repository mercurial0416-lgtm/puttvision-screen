...
object V31OnlineRuntime{
    ...
    fun friendRespond(context:Context,id:String,accept:Boolean,done:(Result<Unit>)->Unit){post(context,"friend-respond",JSONObject().put("id",id).put("accept",accept),true){r->if(r.optBoolean("ok"))done(Result.success(Unit))else done(Result.failure(Exception(r.optString("error","failed"))))}}
    ...
    fun setReady(context:Context,ready:Boolean,done:(Result<Unit>)->Unit){val id=activeRoom?.id?:return done(Result.failure(Exception("room 없음")));post(context,"ready",JSONObject().put("roomId",id).put("ready",ready),true){r->if(r.optBoolean("ok"))done(Result.success(Unit))else done(Result.failure(Exception(r.optString("error","failed"))))}}
...
private fun showV31Friends(context:Context){V31OnlineRuntime.friendList(context){r->r.onSuccess{j->val fs=j.optJSONArray("friendships")?:JSONArray();val ps=j.optJSONArray("players")?:JSONArray();val names=mutableMapOf<String,JSONObject>();for(i in 0 until ps.length()){val p=ps.getJSONObject(i);names[p.optString("id")]=p};val me=V31OnlineRuntime.player?.id.orEmpty();if(fs.length()==0){AlertDialog.Builder(context).setTitle("FRIENDS").setMessage("친구/요청 없음").setPositiveButton("확인",null).show();return@onSuccess};val rows=(0 until fs.length()).map{i->val f=fs.getJSONObject(i);val otherId=if(f.optString("requester_id")==me)f.optString("addressee_id")else f.optString("requester_id");val p=names[otherId];val incoming=f.optString("addressee_id")==me&&f.optString("status")=="pending";val label=when{incoming->"받은 요청";f.optString("status")=="pending"->"보낸 요청";f.optString("status")=="accepted"->"친구";else->f.optString("status")};Triple(f,p,label)};val labels=rows.map{(_,p,label)->"$label · ${p?.optString("display_name")?:"PLAYER"} · R${p?.optInt("rating")?:0}"}.toTypedArray();AlertDialog.Builder(context).setTitle("FRIENDS").setItems(labels){_,idx->val(f,p,_)=rows[idx];val incoming=f.optString("addressee_id")==me&&f.optString("status")=="pending";if(incoming){AlertDialog.Builder(context).setTitle(p?.optString("display_name")?:"친구 요청").setMessage("친구 요청을 수락할까요?").setPositiveButton("수락"){_,_->V31OnlineRuntime.friendRespond(context,f.getString("id"),true){showV31Friends(context)}}.setNegativeButton("거절"){_,_->V31OnlineRuntime.friendRespond(context,f.getString("id"),false){showV31Friends(context)}}.show()}}.setNegativeButton("닫기",null).show()}}}
...
private fun showV31Room(context:Context){V31OnlineRuntime.room(context){r->r.onSuccess{j->val room=j.getJSONObject("room");val players=j.optJSONArray("players")?:JSONArray();val map=mutableMapOf<String,String>();for(i in 0 until players.length()){val p=players.getJSONObject(i);map[p.getString("id")]=p.getString("display_name")};val members=j.optJSONArray("members")?:JSONArray();val me=V31OnlineRuntime.player?.id.orEmpty();var myReady=false;val lines=(0 until members.length()).joinToString("\n"){val m=members.getJSONObject(it);if(m.optString("player_id")==me)myReady=m.optBoolean("ready");"${m.optInt("seat")}. ${map[m.optString("player_id")]?:"PLAYER"} ${if(m.optBoolean("ready"))"READY" else "WAIT"}"};AlertDialog.Builder(context).setTitle("ROOM ${room.optString("join_code")} · ${room.optString("status").uppercase()}").setMessage(lines).setPositiveButton(if(myReady)"READY 해제" else "READY"){_,_->V31OnlineRuntime.setReady(context,!myReady){res->if(res.isSuccess)showV31Room(context)}}.setNeutralButton("새로고침"){_,_->showV31Room(context)}.setNegativeButton("닫기",null).show()}}}
