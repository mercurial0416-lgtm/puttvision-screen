...
data class ShotMetrics(
    ...
    // Physical mat auto-calibration estimate.
...
class ShotTracker {
    ...
    private fun calculate(): ShotMetrics? {
        ...
        val dt = (early.tNs - start.tNs) / 1_000_000_000.0
        if (dt <= 0.0) return null

        val dx = (early.pCm.x - start.pCm.x).toDouble()
        val dy = (early.pCm.y - start.pCm.y).toDouble()
        val distM = hypot(dx, dy) / 100.0
        val ballSpeed = distM / dt
        ...
        val faceToPath =
            if (face != null && path != null) face - path else null

        val smash =
            if (headSpeed != null && headSpeed > 0.05) ballSpeed / headSpeed else null
