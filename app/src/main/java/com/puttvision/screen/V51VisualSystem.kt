...
class V51TvPolishOverlay(context:Context,private val engine:GameEngine):View(context){
    ...
    private fun drawReadDock(c:Canvas){if(engine.state?.running==true||engine.lastResult!=null)return;val read=GreenReadRuntime.peekOrSchedule(engine.settings)?:return;val w=width.toFloat();val h=height.toFloat();val ww=w*.205f;val hh=h*.082f;val right=w*.978f;val left=right-ww;val top=h*.105f;p.color=Color.argb(184,7,11,14);c.drawRoundRect(RectF(left,top,right,top+hh),h*.014f,h*.014f,p);p.typeface=Typeface.DEFAULT_BOLD;p.textSize=max(6.5f,w*.0045f);p.color=Pv.textLo;c.drawText("GREEN READ",left+ww*.07f,top+hh*.26f,p);p.typeface=Typeface.create(Typeface.MONOSPACE,Typeface.BOLD);p.textSize=max(8.2f,w*.0058f);p.color=Pv.textHi;c.drawText("AIM ${read.aimCm}@${read.aimAtM}m",left+ww*.07f,top+hh*.54f,p);p.textSize=max(6.4f,w*.0044f);p.color=Pv.textMid;c.drawText("START ${read.startSpeedMps}m/s  ·  ${read.breakText}",left+ww*.07f,top+hh*.78f,p)}
    private fun drawSubsystemDots(c:Canvas){val w=width.toFloat();val h=height.toFloat();val y=h*.965f;val items=listOf("CAM" to V51VisualPolicy.toneForStatus(engine.camera.status),"CAL" to V51VisualPolicy.toneForStatus(engine.calibration.status),"SHOT" to if(engine.currentShot!=null)V51Tone.GOOD else V51Tone.NEUTRAL,"READ" to if(GreenReadRuntime.peekOrSchedule(engine.settings)!=null)V51Tone.GOOD else V51Tone.WARN);var x=w*.025f;items.forEach{(label,tone)->p.color=v51ToneColor(tone);c.drawCircle(x,y,h*.0045f,p);p.typeface=Typeface.DEFAULT_BOLD;p.textSize=max(5.5f,w*.0038f);p.color=Pv.textMid;c.drawText(label,x+h*.010f,y+h*.004f,p);x+=w*.052f}}
    private fun drawResultRibbon(c:Canvas){val result=engine.lastResult?:return;val w=width.toFloat();val h=height.toFloat();val ww=w*.31f;val hh=h*.057f;val left=(w-ww)/2f;val top=h*.835f;p.color=Color.argb(215,5,10,12);c.drawRoundRect(RectF(left,top,left+ww,top+hh),hh*.32f,hh*.32f,p);p.color=if(result.holed)Pv.primary else Pv.amber;c.drawRect(left,top,left+w*.004f,top+hh,p);p.typeface=Typeface.DEFAULT_BOLD;p.textSize=max(7f,w*.0048f);p.color=Pv.textLo;c.drawText(if(result.holed)"HOLED" else "RESULT",left+ww*.055f,top+hh*.37f,p);p.typeface=Typeface.create(Typeface.MONOSPACE,Typeface.BOLD);p.textSize=max(8.2f,w*.0058f);p.color=Pv.textHi;c.drawText("${result.totalDistanceCm}cm · ${result.breakDirection}",left+ww*.055f,top+hh*.72f,p);p.typeface=Typeface.DEFAULT_BOLD;p.textSize=max(6.4f,w*.0044f);p.color=Pv.textMid;p.textAlign=Paint.Align.RIGHT;c.drawText("${result.elapsedSec}s",left+ww*.95f,top+hh*.62f,p);p.textAlign=Paint.Align.LEFT}}
